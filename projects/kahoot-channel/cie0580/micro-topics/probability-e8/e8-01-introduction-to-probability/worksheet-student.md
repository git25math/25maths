...
## Syllabus focus
- Apply introduction to probability methods accurately in CIE 0580 Extended exam questions.

## Model example
Example: mean of `[9, 4, 7, 17, 13]` is `50/5 = 10.0`.

## Practice (10)
1. A fair die is rolled. Find `P(score > 2)`.
2. If `P(A)=0.35`, find `P(not A)`.
3. Expected frequency for probability `0.3` in `41` trials.
4. If `P(A)=0.4` and `P(B)=0.7` are independent, find `P(A and B)`.
5. If `P(A)=0.4` and `P(B)=0.7` are independent, find `P(A or B)`.
6. A bag has 4 red and 7 blue counters. Find `P(red)`.
