...
## Syllabus focus
- Apply drawing linear graphs methods accurately in Edexcel 4MA1 Higher exam questions.

## Model example
Example: mean of `[6, 16, 5, 5, 17]` is `49/5 = 9.8`.

## Practice (10)
1. Find mean of `[10, 8, 19, 13, 10]`.
2. Find median of `[8, 10, 10, 13, 19]`.
3. Find range of `[10, 8, 19, 13, 10]`.
4. A fair die is rolled. Find `P(score > 3)`.
5. If `P(A)=0.8`, find `P(not A)`.
6. Expected frequency if probability is `0.3` in `69` trials.
