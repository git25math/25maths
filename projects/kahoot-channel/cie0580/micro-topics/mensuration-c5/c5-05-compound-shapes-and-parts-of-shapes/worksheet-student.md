# Edexcel 4MA1 Worksheet (Student)
## C5.5 Compound shapes and parts of shapes

Name: ____________________   Date: ____________________

## Syllabus focus
- Apply compound shapes and parts of shapes methods accurately in Edexcel 4MA1 Higher exam questions.

## Model example
Example: mean of `[9, 5, 8, 10, 16]` is `48/5 = 9.6`.

## Practice (10)
1. Find mean of `[18, 12, 13, 8, 18]`.
2. Find median of `[8, 12, 13, 18, 18]`.
3. Find range of `[18, 12, 13, 8, 18]`.
4. A fair die is rolled. Find `P(score > 1)`.
5. If `P(A)=0.6`, find `P(not A)`.
6. Expected frequency if probability is `0.3` in `66` trials.
7. State which average is least affected by outliers: mean, median, or mode.
8. A class has 12 boys and 18 girls. Choose one at random. Find `P(girl)`.
9. In a two-way table, 14 out of 50 students prefer algebra. Find the probability.
10. If events are independent with `P(A)=0.4` and `P(B)=0.7`, find `P(A and B)`.
