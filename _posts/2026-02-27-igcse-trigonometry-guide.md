...
1. **Label** the triangle: hypotenuse (longest side, opposite the right angle), opposite (across from the angle you're working with), adjacent (next to the angle).
2. **Choose** the ratio that involves the two sides you know (or the side you need).
...
2. **Extract it** — draw it separately in 2D.
3. **Solve** using SOHCAHTOA or Pythagoras.
4. **Repeat** if you need to chain through multiple triangles.
